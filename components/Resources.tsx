...
const Resources: React.FC = () => {
    return (
        <section id="recommended-resources" className="py-20 md:py-32 bg-slate-950 overflow-hidden">
            <div className="container mx-auto px-6 text-center relative z-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 px-4">
                    {books.map((book, index) => (
                        <div 
                            key={index} 
                            className={`group bg-slate-900/70 backdrop-blur-sm p-6 rounded-2xl flex flex-col items-center h-full border border-slate-800 transition-all duration-300 ease-in-out motion-safe:hover:-translate-y-2 hover:shadow-2xl hover:shadow-amber-500/10 hover:border-amber-400/50 animate-fade-in-up`}
                            style={{ animationDelay: `${(index % 3) * 100}ms` }}
                        >
                            <div className="overflow-hidden rounded-xl mb-6 shadow-lg w-full">
                                <img src={book.imageUrl} alt={book.alt} className="w-full h-auto aspect-[2/3] object-cover transition-transform duration-300 ease-in-out group-hover:scale-105"/>
                            </div>
